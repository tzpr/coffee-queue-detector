...
def analyze_queue_picture():
    number_of_persons = 0
    PERSON_CLASS_ID = 1
    queue_state = 'no queue, no one in the coffee machine'
    ...
    if number_of_persons > 1:
        queue_state = f'queue of {number_of_persons} persons'

    if number_of_persons == 1:
        queue_state = 'no queue, just one person'
...
def check_queue():
    ...
    if message_broker.get('queue_state') is not queue_state:
        print(f'Update queue state: {queue_state}')
